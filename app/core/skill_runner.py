...
class SkillRunner:
    ...
    async def extract_draft_json(self, payload: SkillV3InputPayload, few_shot_snippet: str = "") -> Dict[str, Any]:
        ...
        if not cleaned_json_str:
            logger.warning("LLM returned empty or non-JSON content: %r", response_text[:200])
            if self.validate_prompt_template:
                val_prompt = self.build_validate_prompt("", ["LLM output was empty; please extract valid JSON directly."])
                try:
                    repaired_text = await self.call_llm(val_prompt)
                    cleaned_json_str = self._clean_json_response(repaired_text)
        ...
        if cleaned_json_str:
            try:
                ...
                logger.warning("Initial LLM JSON parse failed: %s (Raw content: %r)", jde, cleaned_json_str[:200])
